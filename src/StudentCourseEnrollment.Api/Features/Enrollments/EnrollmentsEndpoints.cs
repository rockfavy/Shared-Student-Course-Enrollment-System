...
public static class EnrollmentsEndpoints
{
        ...
        EnrollmentContext db)
    {
        try
        {
            ...
            var courseForEnrollment = await db.Courses
                .Include(c => c.Enrollments)
                .FirstOrDefaultAsync(c => c.Id == request.CourseId);
            ...
            var currentEnrollments = courseForEnrollment.Enrollments?.Count ?? 0;
            if (currentEnrollments >= courseForEnrollment.Capacity)
            {
                return Results.Problem(
                    title: "Course full",
                    detail: $"This course is full. Capacity: {courseForEnrollment.Capacity} students.",
                    statusCode: 400
                );
            }
