...
public static class CoursesEndpoints
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return Results.Created($"/courses/{createdCourse.Id}", courseDto);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var updatedCourseWithEnrollments = await context.Courses
            .Include(c => c.Enrollments)
            .FirstOrDefaultAsync(c => c.Id == id, cancellationToken);

        var courseDto = new CourseDto(
            updatedCourseWithEnrollments!.Id,
            updatedCourseWithEnrollments.Name,
            updatedCourseWithEnrollments.Description,
            updatedCourseWithEnrollments.Capacity,
            updatedCourseWithEnrollments.Enrollments?.Count ?? 0,
            updatedCourseWithEnrollments.CreatedAt
        );
